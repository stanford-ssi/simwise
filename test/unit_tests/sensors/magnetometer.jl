...
@testset "Magnetometer Sensor Tests" begin
    ...
    std_first = std([v[1] for v in measurements]) <= 1.5e-8 && std([v[1] for v in measurements]) >= 1.2e-8
    std_second = std([v[2] for v in measurements]) <= 1.5e-8 && std([v[2] for v in measurements]) >= 1.2e-8
    std_third = std([v[3] for v in measurements]) <= 1.5e-8 && std([v[3] for v in measurements]) >= 1.2e-8
